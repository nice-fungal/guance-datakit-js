...
LifeCycle.prototype = {
  notify: function notify(eventType, data, options) {
    if (options !== null && options !== void 0 && options.__NO_LOG__) {} else {
  ...
  subscribe: function subscribe(eventType, callback, options) {
...
function validateAndBuildConfiguration(initConfiguration) {
  ...
  return tools_assign({
     ...
     */
    flushTimeout: 5 * tools_ONE_SECOND,
    /**
     * Logs intake limit
     */
...
function batch_trace(level, highlight) {
  var _console;
  var COLOR = {
    'verbose': 'background-color: #F0F9FF',
    'debug': 'background-color: #00F900',
  ...
  for (var _len = arguments.length, message = new Array(_len > 2 ? _len - 2 : 0), _key2 = 2; _key2 < _len; _key2++) {
...
var processedMessageByDataMap = function processedMessageByDataMap(message) {
  batch_trace('debug', '[transport.batch.processedMessageByDataMap]', message);
...
function computeSessionState(configuration, rawTrackingType) {
  ...
  var isSession = performDraw(sessionSampleRate); // 100 true
  var isErrorSession = performDraw(sessionOnErrorSampleRate); // 100 true
  var isSessionReplay = performDraw(sessionReplaySampleRate); // undefined false
  var isErrorSessionReplay = performDraw(sessionReplayOnErrorSampleRate); // undefined false
  var trackingType;
...
function makeRumPublicApi(startRumImpl, recorderApi, options) {
  ...
  var startView = monitor_monitor(function (options) {
  ...
  var rumPublicApi = makePublicApi({
    ...
    addAction: monitor_monitor(function (name, context) {
      console.log('DEMO addAction:', name, context);
