...
var display_globalConsole = console;
var originalConsoleMethods = {};
Object.keys(ConsoleApiName).forEach(function (name) {
  originalConsoleMethods[name] = display_globalConsole[name];
});
...
var display = {
  ...
  warn: originalConsoleMethods.warn.bind(display_globalConsole, PREFIX),
  error: originalConsoleMethods.error.bind(display_globalConsole, PREFIX)
};
...
function defineGlobal(global, name, api) {
  ...
  if (existingGlobalVariable && existingGlobalVariable.q) {
    tools_each(existingGlobalVariable.q, function (fn) {
...
var tools_objectEntries = function objectEntries(object) {
  var res = [];
 ...
 */
function tools_deepClone(value) {
...
function getCorrectedTimeStamp(relativeTime) {
  var correctedOrigin = tools_dateNow() - performance.now();
...
function createValueHistory(params) {
  var __name__ = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '__name__';
...
function computeMessage(stackTrace, isErrorInstance, nonErrorPrefix, originalError) {
  ...
  return stackTrace && stackTrace.message && stackTrace && stackTrace.name ? stackTrace.message : !isErrorInstance ? nonErrorPrefix + ' ' + jsonStringify_jsonStringify(sanitize_sanitize(originalError)) : 'Empty message';
}
...
function buildConsoleLog(params, api, handlingStack) {
  ...
  if (api === ConsoleApiName.error) {
    ...
    error = {
      stack: firstErrorParam ? toStackTraceString(computeStackTrace_computeStackTrace(firstErrorParam)) : undefined,
...
function formatConsoleParameters(param) {
  ...
  if (param instanceof Error) {
    return formatErrorMessage(computeStackTrace_computeStackTrace(param));
  }
...
function createReportObservable(reportTypes) {
  return new Observable(function (observable) {
    ...
    var handleReports = monitor_monitor(function (reports) {
...
function createCspViolationReportObservable(configuration) {
  return new Observable(function (observable) {
    var _addEventListener = addEventListener_addEventListener(document, enums_DOM_EVENT.SECURITY_POLICY_VIOLATION, function (event) {
...
function _trace(level, highlight) {
  var _console;
  var COLOR = {
  ...
  (_console = console).groupCollapsed.apply(_console, ['%c%s', COLOR[level], highlight].concat(message));
  console.trace();
  console.groupEnd();
}
function LifeCycle() {
...
function readBytesFromStream(stream, callback, options) {
  ...
  readMore();
  function readMore() {
    reader.read().then(monitor_monitor(function (result) {
      if (result.done) {
        onDone();
        return;
      }
 ...
 */
function requestIdleCallback_requestIdleCallback(callback, opts) {
 ...
 * performance.
 */
var IDLE_CALLBACK_TIMEOUT = (/* unused pure expression or super */ null && (ONE_SECOND));
...
var SESSION_TIME_OUT_DELAY = 2 * ONE_MINUTE; // 4 * ONE_HOUR
var SESSION_EXPIRATION_DELAY = 1 * ONE_MINUTE; // 15 * ONE_MINUTE
...
function toSessionString(session) {
  return tools_map(tools_objectEntries(session), function (item) {
...
function toSessionState(sessionString) {
  ...
  if (isValidSessionString(sessionString)) {
    sessionString.split(SESSION_ENTRY_SEPARATOR).forEach(function (entry) {
      ...
      if (matches !== null) {
        var _matches = slicedToArray_slicedToArray(matches, 3),
          key = _matches[1],
          value = _matches[2];
        session[key] = value;
      }
...
function expireSessionCookie(options) {
  console.log('%c%s', 'background-color: #FEFB00', '[SessionInCookie.expireSessionCookie]');
  ...
  function synchronizeSession(sessionState) {
    ...
    if (hasSessionInCache()) {
      if (isSessionInCacheOutdated(sessionState)) {
        ...
        console.log('%c%s', 'background-color: #FEFB00', '[SessionStore.sessionStateUpdateObservable.notify]');
...
function createXhrObservable() {
  return new Observable(function (observable) {
    var openInstrumentMethod = instrumentMethod_instrumentMethod(XMLHttpRequest.prototype, 'open', openXhr);
    var sendInstrumentMethod = instrumentMethod_instrumentMethod(XMLHttpRequest.prototype, 'send', function (call) {
      sendXhr(call, observable);
    }, {
      computeHandlingStack: true
    });
    var abortInstrumentMethod = instrumentMethod_instrumentMethod(XMLHttpRequest.prototype, 'abort', abortXhr);
    return function () {
...
function sendXhr(params, observable) {
  ...
  var stopInstrumentingOnReadyStateChange = instrumentMethod_instrumentMethod(xhr, 'onreadystatechange', function () {
  ...
  var unsubscribeLoadEndListener = addEventListener_addEventListener(xhr, 'loadend', onEnd).stop;
...
function createContextManager() {
  ...
  var contextManager = {
    getContext: function getContext() {
      return tools_deepClone(context);
    },
...
function checkUser(newUser) {
  var isValid = tools_getType(newUser) === 'object';
  if (!isValid) {
...
;// CONCATENATED MODULE: ./src/domain/logsCollection/logger/loggerCollection.js
    ...
    if (isAuthorized(logsMessage.status, HandlerType.http, logger)) {
      var rawLogEventData = {
        rawLogsEvent: {
          ...
          origin: errorTools_ErrorSource.LOGGER
        },
...
;// CONCATENATED MODULE: ./src/domain/createErrorFieldFromRawError.js
...
;// CONCATENATED MODULE: ./src/domain/logger.js
...
Logger.prototype = {
  logImplementation: monitor_monitor(function (message, messageContext, status, error, handlingStack) {
    if (status === undefined) {
      status = StatusType.info;
    }
    var sanitizedMessageContext = sanitize_sanitize(messageContext);
    var context;
    if (error !== undefined && error !== null) {
      var rawError = computeRawError({
        stackTrace: error instanceof Error ? computeStackTrace_computeStackTrace(error) : undefined,
        originalError: error,
        nonErrorPrefix: NonErrorPrefix.PROVIDED,
        source: errorTools_ErrorSource.LOGGER,
        handling: enums_ErrorHandling.HANDLED,
        startClocks: clocksNow()
      });
      context = extend2Lev({
    ...
    this.handleLogStrategy({
      message: sanitize_sanitize(message),
      context: context,
      status: status
    }, this, handlingStack);
  }),
...
;// CONCATENATED MODULE: ./src/boot/preStartLogs.js
...
;// CONCATENATED MODULE: ./src/boot/logsPublicApi.js
...
function makeLogsPublicApi(startLogsImpl) {
  ...
  var mainLogger = new Logger(function () {
    var _strategy;
    return (_strategy = strategy).handleLog.apply(_strategy, arguments);
  ...
  return makePublicApi({
    ...
    getGlobalContext: monitor_monitor(function () {
      return globalContextManager.getContext();
    }),
    setGlobalContext: monitor_monitor(function (context) {
      return globalContextManager.setContext(context);
    }),
    setGlobalContextProperty: monitor_monitor(function (key, value) {
      return globalContextManager.setContextProperty(key, value);
    }),
    removeGlobalContextProperty: monitor_monitor(function (key) {
      return globalContextManager.removeContextProperty(key);
    }),
    clearGlobalContext: monitor_monitor(function () {
      return globalContextManager.clearContext();
    }),
    createLogger: monitor_monitor(function (name, conf) {
      if (typeof conf == 'undefined') {
        conf = {};
      }
      customLoggers[name] = new Logger(function () {
        var _strategy2;
        return (_strategy2 = strategy).handleLog.apply(_strategy2, arguments);
      }, customerDataTrackerManager.createDetachedTracker(), sanitize(name), conf.handler, conf.level, sanitize(conf.context));
      return customLoggers[name];
    }),
    getLogger: monitor_monitor(function (name) {
      return customLoggers[name];
    }),
    getInitConfiguration: monitor_monitor(function () {
      return tools_deepClone(strategy.initConfiguration);
    }),
    getInternalContext: monitor_monitor(function (startTime) {
      return strategy.getInternalContext(startTime);
    }),
    setUser: monitor_monitor(function (newUser) {
      if (checkUser(newUser)) {
        userContextManager.setContext(newUser);
      }
    }),
    getUser: monitor_monitor(function () {
      return userContextManager.getContext();
    }),
    removeUserProperty: monitor_monitor(function (key) {
      return userContextManager.removeContextProperty(key);
    }),
    setUserProperty: monitor_monitor(function (key, property) {
      userContextManager.setContextProperty(key, property);
    }),
    clearUser: monitor_monitor(function () {
...
;// CONCATENATED MODULE: ./src/domain/logsSessionManager.js
...
function startLogsAssembly(sessionManager, configuration, lifeCycle, getCommonContext, reportError) {
  ...
  var logRateLimiters = {};
  tools_each(statusWithCustom, function (status) {
  ...
  lifeCycle.subscribe(LifeCycleEventType.RAW_LOG_COLLECTED, function (data) {
    ...
    if (configuration.beforeSend && configuration.beforeSend(log, domainContext) === false || log.origin !== errorTools_ErrorSource.AGENT && tools_isNullUndefinedDefaultValue(logRateLimiters[log.status], logRateLimiters['custom']).isLimitReached()) {
      return;
    }
...
;// CONCATENATED MODULE: ./src/domain/logsCollection/console/consoleCollection.js




var LogStatusForApi = _defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty({}, ConsoleApiName.log, StatusType.info), ConsoleApiName.debug, StatusType.debug), ConsoleApiName.info, StatusType.info), ConsoleApiName.warn, StatusType.warn), ConsoleApiName.error, StatusType.error);
  ...
  var consoleSubscription = initConsoleObservable(configuration.forwardConsoleLogs).subscribe(function (log) {
    lifeCycle.notify(LifeCycleEventType.RAW_LOG_COLLECTED, {
      rawLogsEvent: {
        date: timeStampNow(),
        message: log.message,
        origin: errorTools_ErrorSource.CONSOLE,
...
;// CONCATENATED MODULE: ./src/domain/logsCollection/report/reportCollection.js
  ...
  var reportSubscription = initReportObservable(configuration, configuration.forwardReports).subscribe(function (rawError) {
    ...
    lifeCycle.notify(LifeCycleEventType.RAW_LOG_COLLECTED, {
      rawLogsEvent: {
        date: timeStampNow(),
        message: message,
        origin: errorTools_ErrorSource.REPORT,
        error: error,
        status: status
      }
    });
...
;// CONCATENATED MODULE: ./src/domain/logsCollection/networkError/networkErrorCollection.js
    ...
    function onResponseDataAvailable(responseData) {
      ...
      lifeCycle.notify(LifeCycleEventType.RAW_LOG_COLLECTED, {
        rawLogsEvent: {
          ...
          origin: errorTools_ErrorSource.NETWORK
        },
...
function computeFetchErrorText(error, configuration, callback) {
  callback(truncateResponseText(toStackTraceString(computeStackTrace_computeStackTrace(error)), configuration));
}
function computeFetchResponseText(response, configuration, callback) {
  ...
  if (!clonedResponse || !clonedResponse.body) {
    ...
    // })
    clonedResponse.text().then(monitor_monitor(function (text) {
...
;// CONCATENATED MODULE: ./src/transport/startLogsBatch.js
...
;// CONCATENATED MODULE: ./src/domain/reportError.js
  ...
  return function (error) {
    lifeCycle.notify(LifeCycleEventType.RAW_LOG_COLLECTED, {
      rawLogsEvent: {
        ...
        origin: errorTools_ErrorSource.AGENT,
        status: StatusType.error
      }
    });
...
;// CONCATENATED MODULE: ./src/transport/startLogsBridge.js
...
;// CONCATENATED MODULE: ./src/boot/startLogs.js
...
//   }
// }
;// CONCATENATED MODULE: ./src/entries/main.js



var datafluxLogs = makeLogsPublicApi(startLogs);
